...
### tart.tracing(options)
  ...
  * `options`: _Object_ _(Default: undefined)_ Optional overrides.
    * `dequeue`: _Function_ `function (eventQueue){}` Function that returns next event to be dispatched given an `eventQueue` _(Example: `function (eventQueue){ Array.prototype.push.apply(eventQueue, events); }`)_.
    * `enqueue`: _Function_ `function (eventQueue, events){}` Function that enqueues the new `events` onto the `eventQueue` in place, causing side-effects _(Example: `function (eventQueue, events){ return eventQueue.shift(); }`)_.
